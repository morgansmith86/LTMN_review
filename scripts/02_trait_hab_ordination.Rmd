...
```{r}
data.all.t <- data %>% 
  select(SITEABBR, UID, HAB_ABBR, light.mean.wt, moist.mean.wt, ph.mean.wt, fert.mean.wt) %>% 
...
```{r}
#filter the data
levels(data.all.t$HAB_ABBR)
#the filter function is from dplyr, part of the tidyverse
chart.data <- filter(data.all.t, HAB_ABBR %in% c("b.bog", "r.bog", "fen"))
```
...
```{r}
ggplot(chart.data, aes(x = Index, y = ellval)) +
  geom_polygon(aes(group = UID, color = HAB_ABBR), 
...
### add unique id
```{r}
data.all$UID <- paste(data.all$SITEABBR, data.all$PLOT_ID, sep = "_")
```
...
```{r}
data.all.t <- data.all %>% 
  select(SITEABBR, UID, TARGET_HAB, HAB_ABBR, light.mean.wt, 
...
ggplot(data.all.t, aes(x = Index, y = ellval)) +
  geom_polygon(aes(group = UID, color = HAB_ABBR), 
               fill = NA, size = 0.5, alpha = 1,
               show.legend = FALSE) +
  geom_line(aes(group = UID, color = HAB_ABBR), size = 0.5, alpha = 1,
            show.legend = FALSE) +
  facet_wrap(~ HAB_ABBR) +
  theme(strip.text.x = element_text(size = rel(0.8)),
        axis.text.x = element_text(size = rel(0.8)),
